...
To allow the Android embedder to communicate with the engine exclusively through [`embedder.h`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h), the C-API must be extended to cover Android-specific subsystem requirements. Each proposed extension corresponds directly to uncoupling one of the internal subsystems identified in the dependency audit and empowering a modular target architecture component (`FlutterEmbedderNative`, `JniRouter` / `JniDelegate` / `JvmInvoker`, `AndroidSurfaceControl`, `AndroidPlatformViewsController`, `AndroidMutatorsMapper`, `AndroidHardwareBuffer`, `AndroidVulkanExternalTexture`, `AndroidSemanticsMapper`, `AndroidWindowMetricsMapper`, `AndroidVsyncWaiter`, `AndroidVMInit`, `AndroidEngineGroup`, and [`APKAssetProvider`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/apk_asset_provider.h#L17)).
...
On Android, graphics context creation must be cleanly decoupled from window surface presentation. The embedder initializes or acquires the graphics context (such as Vulkan [`VkInstance`](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkInstance.html), physical device, logical [`VkDevice`](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkDevice.html), [`VkQueue`](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkQueue.html), and Vulkan proc loaders, or EGL display, context, and config) prior to invoking [`FlutterEngineInitialize`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h#L2970).
...
> [!IMPORTANT] No C-ABI extension is proposed for thread merging
> Publishing a thread-merger handle and frame lifecycle callbacks in `embedder.h` would make the engine's internal threading model part of the public ABI and leak `RasterThreadMerger` lease semantics to embedders. Instead, the migration retains [`AndroidExternalViewEmbedder`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/external_view_embedder/external_view_embedder.h) and [`fml::RasterThreadMerger`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml/raster_thread_merger.h#L31) only inside the quarantined legacy target during Phases 2 and 3, and deletes `external_view_embedder/` in Phase 4 once `AndroidPlatformViewsController` routes all platform view composition through `FlutterCompositor` and `AndroidSurfaceControl`.
...
2. **View Departure and Hysteresis**: When the viewport scrolls past the last platform view or the view is dismissed, subsequent frames contain no platform layers. The embedder halts lease extensions. Over the next 10 consecutive frames, the remaining lease count decrements on each frame (9, 8, ..., 0). On the 10th frame after view departure, the lease count reaches 0, `UnMergeNowIfLastOne()` executes, and the rasterizer transitions back to independent multi-threaded execution. This 10-frame lease term provides hysteresis: transient gaps without platform layers (such as dynamic view recycling or brief animations) do not repeatedly merge and unmerge OS threads.

#### Interaction with Static Thread Merging

Flutter supports running with statically merged platform and UI or raster threads via `--merged-platform-ui-thread` or `kMergeAfterLaunch`. When task queues are statically merged, [`RasterThreadMerger`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml/raster_thread_merger.h#L31) lease operations ([`MergeWithLease`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml/raster_thread_merger.h#L43), [`ExtendLeaseTo`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml/raster_thread_merger.h#L70), [`DecrementLease`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml/raster_thread_merger.h#L77), [`UnMergeNowIfLastOne`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml/raster_thread_merger.h#L66)) are safe no-ops and the merged-state queries unconditionally report `true`. Dynamic thread merging composes safely with static merging without requiring separate code branches.

#### Resolution: Retiring Legacy `AndroidExternalViewEmbedder`

Legacy Hybrid Composition under Skia was the only consumer of dynamic raster thread merging in `shell/platform/android/external_view_embedder/`. Under Impeller, platform view composition executes asynchronously via Hybrid Composition++ (HCPP via `SurfaceControl.Transaction` and hardware sync fences on API 34+) and Texture Layer Hybrid Composition (TLHC on lower API levels). In Phase 4, `external_view_embedder/` (`AndroidExternalViewEmbedder` and `AndroidExternalViewEmbedder2`) is removed from `shell/platform/android/`, and `AndroidPlatformViewsController` drives all platform view composition through the public `FlutterCompositor` interface (`create_backing_store_callback`, `collect_backing_store_callback`, and `present_layers_callback`). This eliminates `fml::RasterThreadMerger` from the Android embedder without adding thread-merger handles to `embedder.h` and leaves zero exceptions to the GN compile-time firewall.
...
To resolve this gap, [`embedder.h`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) introduces [`FlutterOpenGLTexture2`](#external-texture-composition-and-surfaceproducer):
...
1. When Dart requests a library, [`FlutterRequestDartDeferredLibraryCallback`](#deferred-component-loading-and-asset-lifecycles) runs asynchronously (`void` return type), so the embedder can initiate download or extraction without blocking the engine thread.
...
- The `:flutter_embedder_native_src` GN source set compiles with `defines = [ "FLUTTER_ENGINE_NO_PROTOTYPES" ]`, ensuring at compile time that all C-API symbols are invoked through the function pointer table rather than direct static symbol linkage.
...
- `FlutterEmbedderNative` (`flutter_embedder_native.cc` / `flutter_embedder_native.h`): Serves as the exclusive top-level JNI coordinator (`FlutterEmbedderNative::RegisterJni(env)` in `library_loader.cc`). It owns the opaque [`FLUTTER_API_SYMBOL(FlutterEngine)`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h#L1523) handle, manages the `FlutterEngineProcTable`, and coordinates the modular subsystem components.
...
- Restrict dependencies strictly to:
  - [`//flutter/shell/platform/embedder:embedder_headers`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/BUILD.gn) and [`//flutter/shell/platform/embedder:embedder_as_internal_library`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/BUILD.gn#L75) (with `defines = [ "FLUTTER_ENGINE_NO_PROTOTYPES" ]`)
  - [`//flutter/fml`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml)
  - [`//flutter/common`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/common)
  - [`//flutter/assets`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/assets)
...
In Phase 2, the Android native source set in [`shell/platform/android/BUILD.gn`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/BUILD.gn) isolates all C-API code in `flutter_embedder_native_src` (`FlutterEmbedderNative`, `JniRouter`, `AndroidSurfaceControl`, `AndroidPlatformViewsController`, `AndroidMutatorsMapper`, `AndroidHardwareBuffer`, `AndroidVulkanExternalTexture`, `AndroidSemanticsMapper`, `AndroidWindowMetricsMapper`, `AndroidVsyncWaiter`, `AndroidVMInit`, `AndroidEngineGroup`, and `APKAssetProvider`). This target sets `defines = [ "FLUTTER_ENGINE_NO_PROTOTYPES" ]` and declares dependencies strictly on [`//flutter/shell/platform/embedder:embedder_headers`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/BUILD.gn), [`//flutter/fml`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/fml), [`//flutter/common`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/common), and [`//flutter/assets`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/assets). Because GN evaluates `check_includes = true` during builds, any attempt to include headers from undeclared targets (such as [`//flutter/flow`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/flow) or [`//flutter/runtime`](https://github.com/flutter/flutter/tree/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/runtime)) causes a compilation error immediately.

#### Layer 2: Monotonic Header Inclusion Ratchet ([`check_android_embedder_deps.py`](#presubmit-header-allowlist-and-monotonic-decrement-ratchet))
...
1. *Zero new files*: Only source files explicitly registered in [`allowed_internal_headers.yaml`](#presubmit-header-allowlist-and-monotonic-decrement-ratchet) may contain internal engine includes. Any inclusion in an unlisted file fails presubmit.
...
1. **Thread-Sensitive Concurrency and Race Conditions**:
   ...
   - Initialization and destruction races: Multi-threaded startup where Flutter engine initialization coincides with rapid [`Activity`](https://developer.android.com/reference/android/app/Activity) destruction to verify thread safety across [`FlutterJNI`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/io/flutter/embedding/engine/FlutterJNI.java#L108), [`ALooper`](https://developer.android.com/ndk/reference/group/looper) callbacks, and native task runners.
...
2. **[`Activity`](https://developer.android.com/reference/android/app/Activity) and Window Lifecycle Transitions**:
   ...
   - Window surface reattachment without engine destruction: Testing that [`FlutterEngineNotifyDestroyed`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) followed by [`FlutterEngineNotifyCreated`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) cleanly rebinds new swapchains and preserves running isolate state without context leaks or corrupted render targets.
...
- Risk: Degradation of Hybrid Composition++ (HCPP) hardware buffer presentation or [`SurfaceControl`](https://developer.android.com/reference/android/view/SurfaceControl) transaction synchronization.
  - Mitigation: HCPP surface pooling and fence handling are ported directly into the modular [`AndroidPlatformViewsController`](#phase-2-in-place-modularization-and-component-decomposition) delegating to [`PlatformViewsController2`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/io/flutter/plugin/platform/PlatformViewsController2.java#L55). DeviceLab benchmarks run side-by-side on Android 14+ devices comparing frame pacing and transaction submission times.
...
- Granularity of [`FlutterPlatformViewMutation`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h#L2071) serialization: Whether complex custom clipping paths should be serialized as vector verbs and points or pre-rasterized masks when passing across the C-ABI.

...
1. **Precedent**: The engine team has already appended members directly to `FlutterSemanticsNode2` without introducing a v3 struct (commit [`59d8010db51`](https://github.com/flutter/flutter/commit/59d8010db51) added `heading_level`, and commit [`f01fa5950c0`](https://github.com/flutter/flutter/commit/f01fa5950c0) added `identifier`).
...
[`embedder.h`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) is extended with callbacks and entry points mirroring the Dart SDK deferred loading API (landed in engine commit [`6fdfc87411d`](https://github.com/flutter/flutter/commit/6fdfc87411d)):
...
1. When Dart requests a library, `FlutterRequestDartDeferredLibraryCallback` runs asynchronously (`void` return type), allowing the embedder to initiate download or extraction without blocking the engine thread.
2. When loading completes, `FlutterLoadDeferredLibraryInfo.destruction_callback` provides the unmap trigger. In [`embedder.cc:3756-3830`](https://github.com/flutter/flutter/blob/6fdfc87411d/engine/src/flutter/shell/platform/embedder/embedder.cc#L3756-L3830), the engine captures this callback in a `DeferredLibraryLifetime` handle held by `fml::NonOwnedMapping`. When the Dart VM finishes loading and drops its references to both data and instruction buffers, the destructor triggers `destruction_callback(user_data)`, allowing the Android embedder to safely `munmap()` or close the underlying `AAsset`.
...
2. This follows the existing precedent in `embedder.h` (commit [`e064e756763`](https://github.com/flutter/flutter/commit/e064e756763)), which appended window constraint fields directly to `FlutterWindowMetricsEvent`.
3. Engine implementations use `SAFE_ACCESS(event, member, default_value)` to read new fields, ensuring that existing desktop embedders passing the baseline struct size continue to work without modification.

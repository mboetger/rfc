...
## Subsystem Deep-Dives: Architectural Requirements and Proposed Embedder C-APIs
...
In the existing engine architecture, `AndroidContextDynamicImpeller` defers the decision between Vulkan and OpenGL ES until `GetImpellerContext` is first called on the raster thread. Because `embedder.h` requires a concrete renderer configuration struct (`FlutterRendererConfig` selecting `kOpenGL` or `kVulkan`) at `FlutterEngineInitialize` time, backend resolution is performed during early embedder initialization (`FlutterLoader` and native context probe) before passing renderer pointers to the engine.
...
Additionally, [`embedder.h`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) receives an API allowing embedders to construct a render target directly from a Vulkan backing store, aligning with Impeller Vulkan swapchain requirements. Window surface attachments (`ANativeWindow`) and swapchains are created and destroyed dynamically across Android Activity lifecycle events via `FlutterEngineNotifyCreated` and `FlutterEngineNotifyDestroyed` (see Renderer Availability and Lifecycle Signals) without destroying the underlying GPU context.
...
Under OpenGL ES, platform view compositing also faces constraints: because `SurfaceControl` transaction sync fences depend on Vulkan hardware synchronization, OpenGL platform views must use Texture Layer Hybrid Composition (TLHC) or legacy Hybrid Composition rather than HCPP.
...
An array of `FlutterAssetResolver` pointers is passed into [`FlutterProjectArgs.custom_asset_resolvers`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h), allowing the engine to stream assets directly from the APK. During hot restart, the engine calls `UpdateAssetResolverByType` to reload asset manifests and updated Dart bytecode bundles without restarting the native engine instance. `FlutterEngineUpdateAssetResolver` allows the embedder to refresh or replace registered resolvers at runtime.
...
The `FlutterEngineSpawnConfig` struct uses the info struct pattern (`struct_size`) to maintain forward ABI compatibility. The `initial_route` parameter matches `Shell::Spawn` and `AndroidShellHolder::Spawn`, allowing spawned isolates to mount distinct route hierarchies immediately upon launch.
...
Android window surfaces are created and destroyed dynamically by the operating system during Activity transitions, configuration changes, or backgrounding. The embedder must coordinate surface availability and GPU context lifecycle without calling protected methods on `PlatformView`.
...
These entry points map to Android `SurfaceHolder.Callback` and Activity lifecycle events:
- `surfaceCreated`: The embedder binds the native window (`ANativeWindow`) and calls `FlutterEngineNotifyCreated` to allocate swapchains and resume rendering.
- `surfaceChanged`: The embedder transmits physical dimension and scale changes to the engine via `FlutterEngineSendWindowMetricsEvent`.
- `surfaceDestroyed`: The embedder calls `FlutterEngineNotifyDestroyed` to drain pending rasterization workloads and destroy window swapchains while preserving engine state, isolate groups, and the GPU device context.
- Activity `onStop` / `onStart`: When an application is backgrounded, the OS may reclaim GPU allocations. The embedder invokes `FlutterEngineSetGpuAvailability(engine, kFlutterGpuAvailabilityUnavailable)` to suspend raster tasks and release volatile GPU buffers without terminating the engine. When the Activity returns to the foreground, `FlutterEngineSetGpuAvailability(engine, kFlutterGpuAvailabilityAvailable)` restores raster scheduling.
...
Unlike legacy Hybrid Composition (see Dynamic Raster Thread Merging), which merges the raster thread into the platform thread to avoid synchronization deadlocks, HCPP delegates layer compositing directly to Android's OS window compositor (SurfaceFlinger) using `SurfaceControl` and `SurfaceControl.Transaction`:
...
2. **Layer Slicing and Dispatch**: During frame presentation, `present_layers_callback` delivers an ordered list of `FlutterLayer` items:
   ...
   - `kFlutterLayerContentTypePlatformView`: Embedded native Android `View` nodes, accompanied by `FlutterPlatformViewIdentifier` and serialized `FlutterPlatformViewMutation` arrays (see Platform View Mutator Stack Serialization).
...
Background plugins that execute Dart entrypoints out of process or without an active `FlutterView` rely on callback registration ([`PluginUtilities.getCallbackHandle`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/lib/ui/plugins/callback_cache.h)). In the C-API architecture, callback cache operations interact with process-scoped utilities rather than individual engine instances, persisting across engine lifecycles.

### Platform Channel Messaging, Background Handlers, and Task Queues

In the existing Android architecture, `PlatformMessageHandlerAndroid::DoesHandlePlatformMessageOnPlatformThread` returns `false`, allowing message dispatch from arbitrary threads, whereas desktop embedders using [`embedder.h`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) return `true` (enforcing platform-thread handling). The Android C-API embedder resolves this discrepancy by decoupling message dispatch from platform-thread affinity.

The Android Java embedding supports executing platform channel handlers on background threads through `BinaryMessenger.TaskQueue`. To preserve this under the C-API, incoming platform messages route directly to the designated background executor pool, and responses are submitted asynchronously via [`FlutterEngineSendPlatformMessageResponse`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h#L3133) without hopping through the Android main thread.
...
Platform-agnostic features use dedicated C-APIs rather than platform channels:
...
2. **Non-Linear Font Scaling**: Android 14+ (API level 34+) implements non-linear font scaling curves, scaling large typography at lower ratios than body text to avoid clipping. Rather than transmitting unstructured JSON across platform channels, font scale curves are encoded directly in [`FlutterWindowMetricsEvent`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h#L1340) for direct ingestion by LibTxt and the framework.
...
For production mobile embedders with extensive test suites, an isolated parallel target isolates new development from continuous integration, allowing defects and drift to accumulate undetected until the eventual cutover.
...
All C-API extensions specified in Subsystem Deep-Dives are implemented in [`embedder.h`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h) and [`embedder.cc`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.cc).
...
1. **Thread-Sensitive Concurrency and Race Conditions**:
   - Surface destruction concurrent with rasterizer frame submission: Native Android `SurfaceView` and `TextureView` lifecycles destroy surfaces asynchronously on the Android UI thread while the rasterizer submits draw calls on the raster thread. Dedicated concurrency tests must simulate rapid window detachment during active frame rendering to catch race conditions and dangling surface handles.
   - Initialization and destruction races: Multi-threaded startup where Flutter engine initialization coincides with rapid Activity destruction, testing thread safety across `FlutterJNI`, `ALooper` callbacks, and native task runners.

2. **Activity and Window Lifecycle Transitions**:
   - Full Activity lifecycle permutations: Explicit tests exercising `onPause` -> `onResume`, `onStop` -> `onRestart` -> `onStart`, and `onDestroy` -> `onCreate` (including configuration changes such as screen rotation, split-screen multi-window mode changes, and foldable device posture folds).
...
3. **Multi-Display and Presentation Surfaces**:
   - Presentation displays: Android applications projecting secondary UI to external displays (via `android.app.Presentation`, wireless casting, or Android Auto) operate multiple window surfaces across distinct refresh rates and display densities. Dedicated integration tests must validate multiple simultaneous embedder instances.

4. **Background Execution and Headless Services**:
   - Background execution without an attached window surface: Testing headless execution inside Android `Service` or `BroadcastReceiver` components (e.g. background work managers, geofencing, background audio), confirming that background platform channels, Dart isolates, and deferred library loaders execute properly when no window surface is bound and GPU resources are unavailable.
...
2. Stage 2: CI Dual-Run Matrix
   - Automated CI runs all test suites ([`flutter_shell_native_unittests`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/BUILD.gn#L48), integration tests, and DeviceLab benchmarks) under a matrix validating both embedder implementations against all four platform view implementations across supported rendering backends:
     ...
     - Platform view implementations (4): Hybrid Composition++ (HCPP via `SurfaceControl` on API 34+), legacy Hybrid Composition (HC via dynamic thread merging), Texture Layer Hybrid Composition (TLHC), and Virtual Displays (VD).
...
If the Flutter project decides to drop Skia support on Android—making Impeller the sole rendering backend—prior to or concurrently with the embedder migration, the implementation complexity, surface area, and testing burden decrease considerably. This section analyzes the specific architectural simplifications enabled by an Impeller-only constraint.
...
Under an Impeller-only model, all direct dependencies on `//third_party/skia` in [`shell/platform/android/BUILD.gn`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/BUILD.gn) are excised immediately in Phase 1 rather than lingering through Phase 5. The starting baseline for the dependency ratchet (The Dependency Ratchet Mechanism) drops from 98 headers to 72 headers on day one.
...
1. Hybrid Composition++ (HCPP, see Hybrid Composition++ (HCPP) and SurfaceControl Compositing) on Android 14+ (API level 34+): Flutter renders its base surfaces and overlay slices onto `AHardwareBuffer`-backed Vulkan swapchains. Multi-surface compositing is delegated directly to Android's OS window compositor (SurfaceFlinger) using `SurfaceControl.Transaction` and hardware sync fences (`ASyncFence`). Rasterization and platform view updates execute independently on separate threads without thread merging or synchronization deadlocks.
...
- Consolidation of `AndroidSurfaceManager` into a unified `ANativeWindow` window management component interfacing with Impeller Vulkan as primary, Impeller OpenGLES as fallback, and `FlutterSoftwareRendererConfig` for headless Google3 test execution.
...
When an Android application is paused, Android captures a thumbnail of the application window for the system task switcher (Recents screen). Under Skia, the embedder maintained [`SnapshotSurfaceProducer`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/surface/snapshot_surface_producer.h#L24) and `CreateSnapshotSurface()`, executing synchronous GPU-to-CPU pixel readbacks ([`GrDirectContext::readPixels`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/third_party/skia/include/gpu/ganesh/GrDirectContext.h) or `SkSurface::makeImageSnapshot`) on the raster thread.

Under Impeller with `SurfaceControl` (HCPP), Android's `SurfaceFlinger` OS compositor captures hardware surface snapshots directly from the hardware buffer tree, eliminating engine-side CPU pixel readbacks and simplifying activity pause handling.
...
- Maintaining legacy `SurfaceTexture` fallbacks in `FlutterRenderer` when `RenderMode.texture` is requested.
...
Skia Ganesh, Skia's Vulkan context manager, Skia's GL interface wrappers, and the Skia software rasterizer represent megabytes of compiled C++ code in `libflutter.so`. Compiling an Impeller-only engine strips these unused Skia compilation units, reducing APK/AAB download size by several megabytes per ABI architecture and reducing runtime dirty memory (RSS) by eliminating Skia's internal texture caches and glyph caches.
...
1. Platform view implementations drop from 4 to 2: Virtual Display (VD) mode and legacy Hybrid Composition (HC with thread merging) are retired entirely, leaving only HCPP (API 34+) and TLHC (API <34).
2. Rendering backends drop from 3 to 2: Skia OpenGLES test suites are eliminated, leaving only Impeller Vulkan and Impeller OpenGLES.

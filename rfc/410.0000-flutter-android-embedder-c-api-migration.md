...
Android supports three platform view composition modes:
...
3. **Texture Layer Hybrid Composition (TLHC)**: Redirects the Android `View` into an offscreen [`SurfaceTexture`](https://developer.android.com/reference/android/graphics/SurfaceTexture) / [`SurfaceProducer`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/android/io/flutter/view/TextureRegistry.java#L21) registered as a Flutter external texture (`createForTextureLayer`).

**Retiring Hybrid Composition (HC) is not an option.** Standard Hybrid Composition remains required across Android devices and API levels where HCPP or TLHC cannot be used. What this migration eliminates is **only the internal C++ `flutter::ExternalViewEmbedder` subclass (`shell/platform/android/external_view_embedder/AndroidExternalViewEmbedder`) and its direct dependencies on `//flutter/flow` (`embedded_views.h`) and `fml::RasterThreadMerger`**, rather than Hybrid Composition itself.
...
#### How Hybrid Composition (HC) Works Through `FlutterCompositor`

In the C-API architecture, the engine's internal [`EmbedderExternalViewEmbedder`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder_external_view_embedder.h) already implements `flutter::ExternalViewEmbedder` behind the public [`FlutterCompositor`](https://github.com/flutter/flutter/blob/4762dec4fc9072cd9269814ef36364f35a9a66dc/engine/src/flutter/shell/platform/embedder/embedder.h#L2117) interface (`create_backing_store_callback`, `collect_backing_store_callback`, and `present_layers_callback` / `present_view_callback`).

`AndroidCompositor` and `AndroidPlatformViewsController` support all three composition modes (`kHybridComposition`, `kHybridCompositionPlusPlus`, and `kTextureLayer`) over `FlutterCompositor` without including internal `//flutter/flow` or `fml::RasterThreadMerger` headers:
...
3. **Removing `external_view_embedder/`**: Because `AndroidCompositor` and `AndroidPlatformViewsController` handle HC overlay blitting, HCPP `SurfaceControl` transactions, and TLHC textures through `FlutterCompositor` while capturing per-frame layer and mutator state by value across threads, `shell/platform/android/external_view_embedder/` (`AndroidExternalViewEmbedder` and `AndroidExternalViewEmbedder2`) is deleted in Phase 4 without removing or degrading Hybrid Composition. No `fml::RasterThreadMerger` handles or internal flow types are exposed in `embedder.h`.

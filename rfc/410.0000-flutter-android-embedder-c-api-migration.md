...
> [!NOTE] Backend Simplification: Dropping Impeller or Dropping Skia
> If Impeller support were dropped on Android, the embedder would only configure OpenGL ES via `FlutterOpenGLRendererConfig` (alongside CPU software rendering). This omits the Vulkan capability probe, Vulkan instance and device lifecycle management, and Vulkan render target extensions. Conversely, if legacy Skia support were dropped in favor of an Impeller-only architecture, all Skia context setup, EGL display and context fallback handling, and Ganesh header dependencies would be eliminated, which narrows graphics initialization to Vulkan with an OpenGL ES fallback.
...
While merged, both platform tasks and raster work execute on the Android platform thread. Once the platform view interaction ceases, the threads unmerge.

> [!NOTE] Composition Simplification: Dropping Legacy Hybrid Composition (HC) or Impeller
> Dynamic thread merging exists solely to prevent rasterization deadlocks in legacy Hybrid Composition (HC) under Skia. If legacy Hybrid Composition is dropped (or if Flutter on Android standardizes entirely on Impeller's asynchronous composition models: HCPP on API 34+ and TLHC below API 34), this entire C-API extension (`FlutterEngineRegisterRasterThreadMerger`, `FlutterRasterThreadMergerConfig`, and `FlutterRasterThreadMergerRef`) is omitted completely from `embedder.h`. Omitting thread merging simplifies `embedder.h`, avoids leaking engine-internal threading assumptions into the public ABI, and eliminates thread-leasing state defects. Conversely, if Impeller were dropped and Skia retained, legacy HC and dynamic thread merging would remain strictly required.
...
> [!NOTE] External Texture Simplification: Dropping Impeller or Dropping Legacy OpenGL
> If Impeller were dropped, the external texture pipeline would rely solely on OpenGL ES textures with UV matrix adjustment (`FlutterOpenGLTexture2`), and would omit `FlutterEngineRegisterVulkanImage` and `AHardwareBuffer` Vulkan import mechanics. Conversely, if legacy OpenGL `SurfaceTexture` support were retired in favor of pure Impeller Vulkan, the embedder could drop `FlutterOpenGLTexture2` and UV coordinate transform matrix conversions, and standardize exclusively on zero-copy `AHardwareBuffer` Vulkan registrations via `FlutterEngineRegisterVulkanImage`.
...
> [!NOTE] Compositor Simplification: Dropping Impeller or Dropping Legacy HC
> If Impeller support were dropped, HCPP (which depends fundamentally on Impeller Vulkan swapchains and GPU sync fences) would be unavailable. The embedder would be forced to rely either on legacy HC (requiring dynamic thread merging) or Texture Layer Hybrid Composition (TLHC), which introduces texture copy overhead. Conversely, if legacy HC were dropped in favor of an Impeller-only architecture, platform view composition would collapse into exactly two asynchronous pipelines: HCPP for Android API 34+ and TLHC for older releases. This completely eliminates dynamic thread leasing, Virtual Display mode, and offscreen Skia canvas allocation.
...
2. **Android 14+ Non-Linear Font Scaling**: Android 14 (API level 34+) uses non-linear font scaling curves so that large typography scales at lower ratios than body text, preventing UI clipping. In the legacy architecture, this was serviced through a synchronous JNI query (`PlatformDispatcher._getScaledFontSize` -> `PlatformView::GetScaledFontSize`), which does not exist in `PlatformViewEmbedder` and triggers `FML_UNREACHABLE()`.
...
To prepare for this transition, Phase 2 decouples all responsibilities bundled inside `PlatformViewAndroid` into five dedicated, single-responsibility components in `flutter_shell_native_capi`:

```mermaid
flowchart LR
    subgraph Monolith["Legacy Monolithic Class: Subclasses flutter::PlatformView"]
        PVA["PlatformViewAndroid<br/>------------------------------------<br/>* Coordinates window lifecycle<br/>* Manages AndroidSurface<br/>* Delegates platform views<br/>* Registers external textures<br/>* Dispatches semantics updates<br/>* Invokes deferred isolate loading"]
    end

    subgraph ModularCAPI["Decomposed Modular Components: Depend on embedder.h"]
        ASM["AndroidSurfaceManager<br/>------------------------------------<br/>Manages ANativeWindow binding,<br/>resizing, and sync surface destruction"]
        APVC["AndroidPlatformViewController<br/>------------------------------------<br/>Coordinates platform view hierarchy,<br/>SurfaceControl, and mutator stacks"]
        ATR["AndroidTextureRegistry<br/>------------------------------------<br/>Manages SurfaceTexture and<br/>AHardwareBuffer external textures"]
        AAB["AndroidAccessibilityBridge<br/>------------------------------------<br/>Converts FlutterSemanticsUpdate2<br/>into Android AccessibilityNodeInfo"]
        AE["AndroidEngine<br/>------------------------------------<br/>Owns FlutterEngine handle,<br/>lifecycle, and isolate entrypoints"]
    end

    subgraph PublicCAPI["embedder.h Public C-ABI Entry Points"]
        ...
        EngineCAPI["FlutterEngineInitialize<br/>FlutterEngineRunInitialized<br/>FlutterEngineLoadDartDeferredLibrary"]
    end

    PVA -. Decomposes into .-> ASM
    PVA -. Decomposes into .-> APVC
    PVA -. Decomposes into .-> ATR
    PVA -. Decomposes into .-> AAB
    PVA -. Decomposes into .-> AE
...
- Alternative 2: Monolithic Cutover without Dual-Delegate Abstraction. Rejected because attempting a single large-scale replacement of `AndroidShellHolder` and `PlatformViewAndroid` without a phased runtime fallback increases regression risk, prevents side-by-side CI verification, and makes triaging defects significantly harder.

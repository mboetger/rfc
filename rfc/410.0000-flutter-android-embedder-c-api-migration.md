...
Dropping Skia enables deprecating the `EnableImpeller` manifest tag, removing `isImpellerEnabled()` branching, and standardizing Java `FlutterRenderer` on `SurfaceProducer` and hardware buffers.

### Substantial Binary Size and Memory Footprint Reduction
...
Because Impeller is fundamentally a GPU-native architecture targeting Vulkan and OpenGLES, it does not include a native CPU software rasterizer. Running Impeller on CPU workers requires software Vulkan emulation such as SwiftShader, which introduces substantial JIT compilation overhead, higher memory footprints, and CPU instruction set requirements (e.g. AVX) that may not be available across all Borg worker pools.

Importantly, preserving software rendering for Google3 headless tests does not require retaining direct dependencies on Skia headers in the Android embedder C++ layer:
...
Dropping Skia simplifies this testing matrix along two independent axes:

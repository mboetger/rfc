...
- **Zero-Initialization Safety**: Existing desktop embedders (Linux, macOS, Windows) zero-initialize `FlutterProjectArgs`, retaining `true` by default and preserving their existing single-threaded platform message assumptions.
- **Mobile Embedder Parity**: Both Android and iOS explicitly set `does_handle_platform_messages_on_platform_thread = false`. On iOS, `PlatformMessageHandlerIos::DoesHandlePlatformMessageOnPlatformThread()` returns `false`, dispatching directly to custom task queues or background GCD dispatch queues. On Android, incoming messages delivered to `FlutterPlatformMessageCallback2` on the UI thread inspect registered channel handlers in `DartMessenger`:
...
3. **Strict Build Boundary**: The target components depend solely on the single GN target `//flutter/shell/platform/embedder:embedder`, reducing Android build graph complexity and eliminating compilation coupling.

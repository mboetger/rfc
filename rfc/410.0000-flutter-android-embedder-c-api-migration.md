...
4. **Phase 3 Cutover Ready**: In Phase 3, when `--android-embedder-api=true` is supplied, `EmbedderAndroidEngineDelegate` activates, cutting over runtime execution from delegating to `Shell` via `PlatformViewAndroid` to calling `embedder.h` C-APIs directly on `FlutterEngine`.

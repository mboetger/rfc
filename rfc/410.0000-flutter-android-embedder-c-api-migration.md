...
sequenceDiagram
    ...
    participant SC as SurfaceControl.Transaction (Java/NDK)
    ...
    Engine->>Embedder: present_layers_callback(layers, count) with synchronization_fence_fd
    Embedder->>SC: setBuffer(surfaceControl, hardwareBuffer, fenceFd)
    Embedder->>SC: applyTransactions() / swapTransactions()
    SC->>SF: Commit transaction with acquire fence
    SF->>SF: GPU waits on acquire fence before compositing
    SF-->>Embedder: Release fence signaled (buffer reading complete)
